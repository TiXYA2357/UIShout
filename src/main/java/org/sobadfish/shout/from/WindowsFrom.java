...
public class WindowsFrom implements Listener {
    ...
    private FromDataListener dataListener;

    private static final int ID = (int)(Math.random()*25565);

    public void setFromDataListener(FromDataListener dataListener){
    ...
    public void onFromListener(PlayerFormRespondedEvent event){
        ...
        if(event.getResponse() instanceof FormResponseCustom && event.getFormID() == ID){
            ...
            if(dataListener != null){
                ...
                dataListener.onData(player,data);
